...
for dataName = p3_dataName_list
    ...
    tmp2 = get_window(cCSI(startIdx:startIdx+floor(test_ratio*n),:,:,:),n_timestamps,step_size);
    tmp3 = get_window(cCSI(startIdx+floor(test_ratio*n)+1:end,:,:,:),n_timestamps,step_size);
    train_CSI_presence3 = cat(1,train_CSI_presence3,tmp1,tmp3);
    test_CSI_presence3 = cat(1,test_CSI_presence3,tmp2);
...
Y_train = cat(1,ones(size(X_train_CSI_empty,1),1),ones(size(X_train_CSI_empty,1),1),2*ones(size(X_train_CSI_presence1,1),1),3*ones(size(X_train_CSI_presence2,1),1),4*ones(size(X_train_CSI_presence3,1),1));
CSI_train = cat(1,X_train_CSI_empty,X_train_CSI_empty,X_train_CSI_presence1,X_train_CSI_presence2,X_train_CSI_presence3);
% Y_train = categorical(Y_train);
CSI_test = cat(1,X_test_CSI_empty,X_test_CSI_presence1,X_test_CSI_presence2,X_test_CSI_presence3);
Y_test = cat(1,ones(size(X_test_CSI_empty,1),1),2*ones(size(X_test_CSI_presence1,1),1),3*ones(size(X_test_CSI_presence2,1),1),4*ones(size(X_test_CSI_presence3,1),1));
